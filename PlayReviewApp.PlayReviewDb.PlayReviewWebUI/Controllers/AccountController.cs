...
namespace PlayReviewApp.PlayReviewDb.PlayReviewWebUI.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login([FromForm] LoginModel loginModel)
        {
            ...
            {
                IdentityUser user = await _serviceManager.UserManager.FindByNameAsync(loginModel.UserName);
                ...
                await _serviceManager.SignInManager.SignOutAsync();
                var result = await _serviceManager.SignInManager.PasswordSignInAsync(user, loginModel.Password, false, false);
        ...
        public async Task<IActionResult> Register([FromForm] RegisterDto model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _serviceManager.UserManager.CreateAsync(newUser, model.Password);

                if (result.Succeeded)
                {
                    var roleResult = await _serviceManager.UserManager.AddToRoleAsync(newUser, "user6");
        ...
        {
            var identityUser = await _serviceManager.UserManager.GetUserAsync(User);
